...
from test_suite.universal_test_functions import (
    ...
    signal_detection_test, periodicity_test, clustering_analysis,
...
def load_real_datasets():
    ...
    # 4. Enzyme Data (Real biological data)
    enzyme_files = {
        'wild_type': Path("data/wild_type_enzyme.fasta"),
        'mutant': Path("data/mutant_enzyme.fasta"),
        'structure': Path("data/example_enzyme.pdb")
    }
    
    if all(f.exists() for f in enzyme_files.values()):
        datasets['enzyme'] = {
            'data': enzyme_files,
            'name': 'PETase Enzyme Dataset',
            'description': 'Real enzyme sequences and structure data',
            'shape': 'Multiple files (FASTA + PDB)',
            'source': 'PETase S238F mutant with experimental validation'
        }
        print(f"✅ Loaded Enzyme dataset: Multiple files")
...
def analyze_real_dataset(dataset_name, dataset_info):
    """Analyze a real dataset using all available test functions."""
    print(f"\n{'='*60}")
    ...
    print(f"\n5. Clustering Analysis")
    print("-" * 40)
    clustering_result = clustering_analysis(data)
    results['clustering_analysis'] = clustering_result
    
    if 'metrics' in clustering_result:
        metrics = clustering_result['metrics']
        print(f"  - Optimal clusters: {metrics.get('optimal_clusters', 'N/A')}")
        print(f"  - Silhouette score: {metrics.get('silhouette_score', 'N/A')}")
    
    # 6. Dimensionality Analysis
    print(f"\n6. Dimensionality Analysis")
    print("-" * 40)
    dim_result = dimensionality_analysis(data)
    results['dimensionality_analysis'] = dim_result
    
    if 'metrics' in dim_result:
        metrics = dim_result['metrics']
        print(f"  - Explained variance: {metrics.get('explained_variance', 'N/A')}")
        print(f"  - Optimal components: {metrics.get('optimal_components', 'N/A')}")
    
    # 7. Domain-specific analysis for LIGO
    if dataset_name == 'ligo':
        print(f"\n7. LIGO-specific Analysis")
        print("-" * 40)
    ...
    # 8. Domain-specific analysis for Enzyme
    if dataset_name == 'enzyme':
        print(f"\n7. Enzyme-specific Analysis")
        print("-" * 40)
        
        # Analyze wild-type sequence
        wild_type_file = str(data['wild_type'])
        enzyme_result = enzyme_sequence_analysis(wild_type_file)
        results['enzyme_analysis'] = enzyme_result
        
        if 'metrics' in enzyme_result:
            metrics = enzyme_result['metrics']
            print(f"  - Sequence length: {metrics.get('sequence_length', 'N/A')}")
            print(f"  - Molecular weight: {metrics.get('molecular_weight', 'N/A'):.1f}")
    
    return results

def run_pipeline_integration_with_real_data():
    """Run pipeline integration with real datasets."""
    print(f"\n{'='*60}")
    print("PIPELINE INTEGRATION WITH REAL DATA")
    print(f"{'='*60}")
...
def create_comprehensive_report(datasets, individual_results, pipeline_results):
    """Create a comprehensive report of all real data analysis."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    report = {
        "timestamp": timestamp,
        "analysis_type": "real_data_integration",
        "datasets_analyzed": list(datasets.keys()),
        "individual_results": individual_results,
        "pipeline_results": pipeline_results,
        "summary": {
            "total_datasets": len(datasets),
            "total_tests_per_dataset": 6,  # statistical, correlation, signal, periodicity, clustering, dimensionality
            "real_data_sources": [
                "Iris Flower Dataset (Fisher, 1936)",
                "Titanic Passenger Dataset (Historical)",
                "LIGO GW150914 Dataset (Gravitational Wave)",
                "PETase Enzyme Dataset (Biological)"
            ]
        }
    }
    
    report_file = f"real_data_integration_report_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        if not datasets:
            ...
            print("  - data/wild_type_enzyme.fasta")
            return
        ...
        # Individual analysis
        print("\n" + "="*60)
        print("INDIVIDUAL REAL DATA ANALYSIS")
        print("="*60)
        
        individual_results = {}
        for dataset_name, dataset_info in datasets.items():
            results = analyze_real_dataset(dataset_name, dataset_info)
            individual_results[dataset_name] = results
        
        # Pipeline integration
        pipeline_results = run_pipeline_integration_with_real_data()
        
        # Create comprehensive report
        report_file = create_comprehensive_report(datasets, individual_results, pipeline_results)
        
        print("\n" + "="*60)
        print("REAL DATA INTEGRATION COMPLETED")
        print("="*60)
        ...
        print(f"  - Tests per dataset: 6")
        print(f"  - Total analyses: {len(datasets) * 6}")
